...
### 진행 중 작업 인계 (필수)
긴 작업(여러 turn 걸리는 코드 작성, 보고서, 멀티 단계 워크플로 등)이 진행 중일 때는 매 turn마다 또는 사용자가 "새 세션 시작", "다시 시작", "잠시만" 등 중단/세션 전환 시사 시 **반드시** 현재 상태를 메모리에 저장해:
- 파일: `/home/node/.openclaw/memory/in_progress_<주제slug>.md`
- 내용: ① 작업 목표 ② 지금까지 완료한 단계 ③ 다음 단계 ④ 핵심 결정/파일 경로 ⑤ 미해결 이슈
- 세션 시작 시 memory_search로 `in_progress` 키워드 검색해서 이어갈 작업 있는지 확인. 있으면 사용자에게 "지난 작업 ○○ 이어서 진행할까요?" 먼저 묻고 계속.
- 작업 완료되면 그 파일을 삭제하거나 `done_<주제>.md`로 rename해서 완료 표시.

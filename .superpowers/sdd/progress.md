...
## 결정 번복 (2026-08-12): 상시 업무 칩 제거
- 5.2 "빠른 실행 버튼 노출" 철회 — 에이전트 증가 시 칩 증식(확장성 파산) + "비서가 알아서" 원칙 모순
- 진입 = 자연어(keyword_map+되묻기) · 반복 업무 = 푸시(cron·알림) · 버튼 = 대화 내 카드 한정
- 플랜 Task 6 에서 QuickActions 수정 제거
- Task 2 complete (259a690) — 사업 마스터 + 설정 API, 엔드포인트 실검증
